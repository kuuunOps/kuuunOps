...
EOF
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname=${WORKERS[$i]},${WORKER_IPS[$i]} -profile=kubernetes ${WORKERS[$i]}-csr.json | cfssljson -bare ${WORKERS[$i]}
done
```
---
...
# MASTER_IPS=172.20.10.11,172.20.10.12,172.20.10.13
